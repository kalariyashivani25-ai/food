...
export default function Login({ onToggleAuth }) {
  const { login, googleLoginMock, error: authError } = useAuth();
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      await googleLoginMock(
        'Wellness Explorer',
        'explorer@nutriai.com',
        'google_mock_123456789'
      );
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 px-4 relative overflow-hidden">
      ...
      <div className="absolute bottom-1/4 right-1/4 w-[500px] h-[500px] bg-glow-rose-radial rounded-full pointer-events-none translate-x-1/2 translate-y-1/2"></div>
      ...
      <div className="w-full max-w-md glass-panel p-8 rounded-3xl brand-glow relative z-10 animate-slide-up">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="text-3xl font-bold font-display text-slate-800 dark:text-white">
            Welcome to <span className="gradient-text">NutriAI</span>
          </h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1.5 text-center">
            Your premium AI companion for health, diet, and wellness
          </p>
        </div>

        {error && (
          <div className="mb-6 p-4 rounded-xl bg-rose-500/15 border border-rose-500/20 text-rose-500 text-xs">
            {error}
          </div>
        )}
        ...
        <div className="relative flex py-5 items-center">
          <div className="flex-grow border-t border-slate-200/60 dark:border-slate-800/50"></div>
          <span className="flex-shrink mx-4 text-slate-400 dark:text-slate-500 text-[10px] uppercase font-semibold tracking-widest">or continue with</span>
          <div className="flex-grow border-t border-slate-200/60 dark:border-slate-800/50"></div>
        </div>
        ...
        <button
          ...
          type="button"
          className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200 py-3 rounded-xl text-sm font-semibold hover:bg-slate-50 dark:hover:bg-slate-800/80 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
        >
          {/* Flat Styled Google Icon */}
